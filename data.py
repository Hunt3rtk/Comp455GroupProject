...
data = pd.read_csv("itineraries.csv", chunksize=5000, nrows=10000, usecols=['legId', 'flightDate', 'startingAirport', 'destinationAirport', 'travelDuration', 'isBasicEconomy', 'isRefundable', 'isNonStop', 'totalFare', 'seatsRemaining']) 
...
def next_page(index, page):
    ind = index+1
    pg = page+1
    return render_template('table.html', tables=dataArray, page=pg, index=ind, titles=[''])
